...
def _log_type_mismatch(cls, value) -> None:
    get_logger().warning(
        f"Class `{cls.__name__}` received an invalid and unexpected value `{value}`, a new enum item will be created to represent this value. Please update flux.py or report this issue on GitHub - https://github.com/interactions-py/interactions.py/issues"
    )
...
class UserFlags(DiscordIntFlag):  # type: ignore
    ...
    """Bot uses only HTTP flux and is shown in the online member list"""
