...
class Console(Extension):
    ...
    Args:
        ...
        **kwargs: The locals to make available in the console, by default this includes `client`, `bot` and `Interfluxer`

    """

    def __init__(self, client: Interfluxer.Client, port: int = 501, console_port: int = 502, **kwargs) -> None:
    ...
    async def async_start_bot(self, token: str | None = None) -> None:
        ...
        old_start = Interfluxer.Client.astart

        _locals = self.locals | {
            "client": self.client,
            "bot": self.client,
            "Interfluxer": Interfluxer,
        }
