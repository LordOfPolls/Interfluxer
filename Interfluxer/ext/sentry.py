...
And then call `bot.load_extension('Interfluxer.ext.sentry', dsn=SENTRY_DSN)`
...
from Interfluxer.api.events.internal import Error
from Interfluxer.client.const import get_logger
from Interfluxer.models.internal.tasks.task import Task

try:
    import sentry_sdk
except ModuleNotFoundError:
    get_logger().error(
        "sentry-sdk not installed, cannot enable sentry integration.  Install with `pip install external-py-Interfluxer[sentry]`"
    )
    raise

from Interfluxer import Client, Extension, listen
