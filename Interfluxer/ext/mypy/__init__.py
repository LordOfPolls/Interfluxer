...
To use it, add `Interfluxer.ext.mypy` to the plugins list in your mypy config.

For mypy.ini:
```ini
[mypy]
plugins = Interfluxer.ext.mypy
```
...
class NaffPlugin(Plugin):
    ...
    def get_class_decorator_hook(self, fullname: str) -> Optional[Callable[[ClassDefContext], None]]:
        if fullname == "Interfluxer.client.utils.attr_utils.define":
            return partial(
