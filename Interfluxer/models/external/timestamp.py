...
if TYPE_CHECKING:
    from Interfluxer.models.external.snowflake import Snowflake_Type, Snowflake
...
class Timestamp(datetime):
    ...
    def to_snowflake(self, high: bool = False) -> Union[str, "Snowflake"]:
        ...
        """
        fluxer_millis = int(self.timestamp() * 1000 - FLUXER_EPOCH)
        return (fluxer_millis << 22) + (2**22 - 1 if high else 0)
    ...
    def from_snowflake(cls, snowflake: "Snowflake_Type") -> "Timestamp":
        ...
        ??? Info
            https://fluxer.app/developers/docs/reference#convert-snowflake-to-datetime

        """
        if isinstance(snowflake, str):
            snowflake = int(snowflake)

        timestamp = ((snowflake >> 22) + FLUXER_EPOCH) / 1000
