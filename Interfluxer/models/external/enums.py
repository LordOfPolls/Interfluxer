...
class FluxerIntFlag(IntFlag, metaclass=DistinctFlag):
...
def _log_type_mismatch(cls, value) -> None:
    get_logger().warning(
        f"Class `{cls.__name__}` received an invalid and unexpected value `{value}`, a new enum item will be created to represent this value. Please update Interfluxer or report this issue on GitHub - https://github.com/LordOfPolls/Interfluxer/issues"
    )
...
class Intents(FluxerIntFlag):  # type: ignore
    """
...
class UserFlags(FluxerIntFlag):  # type: ignore
    ...
    FLUXER_CERTIFIED_MODERATOR = 1 << 18
...
class ApplicationFlags(FluxerIntFlag):  # type: ignore
...
class MessageFlags(FluxerIntFlag):  # type: ignore
    """Message flags."""
    ...
    SHOULD_SHOW_LINK_NOT_FLUXER_WARNING = 1 << 10
...
class Permissions(FluxerIntFlag):  # type: ignore
...
class SystemChannelFlags(FluxerIntFlag):
...
class ChannelFlags(FluxerIntFlag):
    PINNED = 1 << 1
...
class ActivityFlag(FluxerIntFlag):
    INSTANCE = 1 << 0
    JOIN = 1 << 1
    SPECTATE = 1 << 2
    JOIN_REQUEST = 1 << 3
...
class MemberFlags(FluxerIntFlag):
    DID_REJOIN = 1 << 0
