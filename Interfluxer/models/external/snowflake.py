...
from Interfluxer.client.const import MISSING, Absent, FLUXER_EPOCH
...
def to_snowflake(snowflake: Snowflake_Type) -> "Snowflake":
    """
    Helper function to convert something into correct Fluxer snowflake int, gives more helpful errors Use internally to sanitize user input or in user- facing APIs (a must).

    For Fluxer-API - facing code, just int() is sufficient
    ...
    if 22 > snowflake.bit_length() > 64:
        raise ValueError(
            f"ID (snowflake) is not in correct Fluxer format! Bit length of int should be from 22 to 64 "
...
class Snowflake(int):
    ...
    def from_datetime(cls, dt: datetime.datetime, *, high: bool = False) -> "Snowflake":
        ...
        ms = int(timestamp * 1000 - FLUXER_EPOCH)
...
class SnowflakeObject:
    id: Snowflake = attrs.field(repr=True, converter=Snowflake, metadata={"docs": "Fluxer unique snowflake ID"})
